import type {ApprovalDecision, ApprovalRequest} from './bash.js';
...
export function webSearchTool(env: WebEnv): ToolDef {
	return {
		spec: {
			name: 'web_search',
			description:
		...
		handler: async (args, signal) => {
			...
			const decision = await approveWeb(env, {
				tool: 'web_search',
				command: query,
...
export function webFetchTool(env: WebEnv): ToolDef {
	return {
		spec: {
			name: 'web_fetch',
		...
		handler: async (args, signal) => {
			...
			return executeWebFetch({url, prompt: String(args.prompt ?? ''), maxChars: clampNumber(args.max_chars, 6000, 500, 20_000)}, signal ?? new AbortController().signal);
		}
	};
}
...
async function approveWeb(env: WebEnv, req: ApprovalRequest): Promise<ApprovalDecision> {
	if (!env.approve) {
		return {approved: false, always: false};
	}
	return env.approve(req);
}
	...
	for (const block of blocks) {
		...
		const snippet = cleanHtml(
			/class="[^"]*result__snippet[^"]*"[^>]*>([\s\S]*?)<\/a>/i.exec(block)?.[1] ??
				/class="[^"]*result__snippet[^"]*"[^>]*>([\s\S]*?)<\/div>/i.exec(block)?.[1] ??
				''
		);
		if (url && title) {
